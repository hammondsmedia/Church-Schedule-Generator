...
export default function CalendarTab({ 
  selectedMonth, 
  setSelectedMonth, // RESTORED: Fixed ReferenceError crash
  schedule, 
  serviceSettings, 
  userRole, 
  setAssigningSlot, 
  ...
  const ServiceBar = ({ type, dk, date }) => {
    ...
    const colors = {
      ...
      communion: { bg: '#fce7f3', text: '#be185d' },
    ...
    return (
      <button 
        ...
        style={speakerName ? { background: color.bg, color: color.text } : {}}
        onClick={() => {
          ...
          return (
            <div key={idx} style={{ 
              ...
              <div style={{ 
                ...
                fontWeight: '800', 
                marginBottom: '8px', 
                color: isToday ? '#1e3a5f' : '#64748b',
                display: 'flex',
                justifyContent: 'space-between'
              }}>
                <span style={isToday ? { background: '#1e3a5f', color: 'white', width: '22px', height: '22px', borderRadius: '50%', display: 'grid', placeItems: 'center', fontSize: '11px' } : {}}>
                  {date.getDate()}
                </span>
              </div>
